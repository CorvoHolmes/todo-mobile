...
export default function Home() {
  ...
  function handleAddTodo() {
    ...
    if (!name) return Alert.alert("To-do sem descrição", "Digite uma descrição da sua tarefa na caixa de texto")
  ...
  function handleRemoveTodos(title: string) {
    ...
    <View className="w-full flex-1 bg-zinc-800 mt-16">
      ...
      <View className="p-6 pb-28">
        <View className="flex-row justify-between mb-5">
          ...
          <View className="flex-row items-center gap-2">
            <Text className="text-violet-400 font-bold">Concluidas</Text>
            <Text className="bg-zinc-600 font-bold text-zinc-300 py-0.5 px-2 rounded-sm">
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Task 
              todo={item}
              onRemove={() => handleRemoveTodos(item.title)}
